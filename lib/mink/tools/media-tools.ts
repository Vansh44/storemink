...
const generateStorefrontImage: MinkTool = {
  declaration: {
    ...
    description:
      "Create one charged, high-resolution AI storefront campaign image or blog cover, save it immediately in this store's Media Library, and return its exact URL. A request to create a blog includes one blog cover by default unless the merchant supplied or explicitly chose an existing image. BEFORE every generation, read the source that matches the request: search_products/get_current_product for a named or selected product, search_storefront_categories for a named category, and list_storefront_media for a particular saved image or for general store visual context. Pass up to four exact relevant URLs from those results in reference_image_urls; pass [] only after the relevant read found no suitable image. The image model receives those files, not merely their URLs. A named product or category must be grounded in its authentic images rather than replaced with an invented stand-in; a specific saved image must guide the new composition exactly as the user requested. Infer purpose from the requested destination: hero for the top/lead area, gallery for a square tile, feature for an image beside text, banner for a full-width promo strip, and blog_cover for a 16:9 editorial article cover. Write a complete creative brief in prompt: central idea, visual hierarchy, subject placement, setting, supporting props, background, depth, lighting, palette, mood and crop-safe negative space. A blog cover must express the article's central idea as one coherent editorial scene and must not contain the title in the pixels. For a sale or offer, visually communicate the occasion with an original, catchy campaign composition (for example paired product rhythm for buy-one-get-one) rather than pasting a cutout onto a plain background; keep all offer wording editable outside the image. The result is AI-generated campaign/editorial artwork and cannot be written to a product's catalogue images. If the request also asks for a blog, immediately use the returned URL as cover_image_url in propose_blog_draft in the same run; stopping here or asking the merchant to attach it manually is incomplete. If it names a page destination, use the returned URL in propose_storefront_layout in the same run when available. Do not add new text, watermarks, signatures or people. Do not pass arbitrary web URLs, guessed URLs, unrelated products, or references the read tools did not return.",
    parametersJsonSchema: {
      ...
      properties: {
        ...
        prompt: {
          ...
          description:
            "A complete creative brief in plain English: central idea, visual hierarchy, full-subject placement, setting, supporting props, depth, lighting, palette, mood and crop-safe negative space. For a blog cover, translate the article's theme into one coherent editorial scene without title text. For an offer, describe an original promotional composition that communicates its energy visually without putting offer text in the pixels. Never request a simple pasted cutout unless the merchant explicitly asked for one.",
        },
