...
import {
  ...
  getLatestMinkStorefrontLayoutAction,
  previewMinkStorefrontLayoutAction,
} from "@/lib/mink/storefront-layout-actions";
 ...
 *
 * ★ WITHOUT IT A RESTORED CARD LIES BY OMISSION. The artifact persists in the
 * conversation, so after a reload the merchant sees "Review Builder draft
 * save" for a layout that was applied an hour ago — and pressing it produces a
 * page-changed conflict, which is safe and completely unexplanatory. Phase 7B
 * gets this for free from its preview DTO; a layout card needs no such DTO, so
 * it asks this one question instead.
 */
export async function GET(
  ...
  if (!UUID_PATTERN.test(draftId)) {
    return privateJson(
      { error: "Invalid Mink AI storefront layout draft." },
      400,
    );
  }
  ...
  try {
    ...
    const limited = await rateLimit(
      `mink-storefront-layout-read:${actor.storeId}:${actor.adminId}`,
    ...
    return privateJson({
      result: await getLatestMinkStorefrontLayoutAction(actor, draftId),
    });
  } catch (error) {
    if (error instanceof MinkRequestError) {
      // ★ A restored card asking about a draft that no longer validates is not
      //   an error worth showing: it simply has nothing to report.
      logWarn("mink.storefront_layout_action: read rejected", {
        requestId,
        draftId,
        code: error.code,
      });
    ...
    logError("mink.storefront_layout_action: read failed", error, {
      requestId,
      draftId,
    });
    return privateJson(
      { error: "Mink AI couldn't load that layout proposal." },
      503,
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const limited = await rateLimit(
      `mink-storefront-layout-action:${actor.storeId}:${actor.adminId}`,
    ...
    if (error instanceof MinkRequestError) {
      logWarn("mink.storefront_layout_action: request rejected", {
        requestId,
        draftId,
    ...
    logError("mink.storefront_layout_action: request failed", error, {
      requestId,
      draftId,
      action: mutation.action,
    });
    return privateJson(
      { error: "Mink AI couldn't complete that storefront layout action." },
      503,
    );
  }
}
