...
- **`lib/mink/storefront-media-read.ts` — `list_storefront_media`.** Bounded at
  40, newest first, with an optional bounded filename search so an older named
  asset is not hidden by pagination. It returns `media_assets.url` VERBATIM (it
  is the string a proposal must echo back, so it is neither shortened nor
  re-derived from the bucket path) plus filename, type, size. `search_products` and
...
- **★★ SAFETY HERE IS STRUCTURAL, NOT A WORD FILTER.** A generated picture must
  ...
  a validated grounding/output contract, exact current-store references and no
  people at all. **Adding a
...
- **★★ THE GENERATION CONTRACT AND EVERY SAFETY LEVER ARE FIXED.** The marked
  runtime block in `docs/mink-ai-image-prompt.md` takes no caller contribution
  beyond exactly one `{{scene_description}}` and one server-authored
  `{{reference_guidance}}`; `lib/mink/image-prompt.ts` validates its markers,
  fence and placeholders before rendering it. Its grounding and output rules
  are shipped through Next output tracing and the Docker-context exception.
  The code separately
  fixes `imageConfig.personGeneration: ALLOW_NONE`, the prominent-person block
...
- **★ THE TOOL AND VALIDATOR ACCEPT THE SAME WIRE SHAPE.** The declaration
  exposes `purpose`, `prompt`, `reference_image_urls` and `alt`; requiring an
  internal schema version from the model previously made every generation fail
  before reaching the provider. An absent version means the current schema,
  while a present wrong version still fails closed.
...
- **EVERY GENERATION STARTS WITH THE RELEVANT CURRENT-STORE IMAGE READ.** A
  named or selected product uses `search_products`/`get_current_product`; a
  named category uses the new `search_storefront_categories`; a particular
  saved image or general visual context uses `list_storefront_media`, including
  its filename search when the merchant names a particular saved asset. The
  generation tool requires `reference_image_urls`, even when the reads found
  none and the value is `[]`, so the model must make the evidence decision
  explicitly.
- **THE PROVIDER RECEIVES THE PICTURES, NOT JUST THEIR URL STRINGS.** The model
  may select at most four exact PNG/JPEG/WebP URLs. Before the credit reservation
  and provider call, `media-reference-images.ts` rechecks the current tenant,
  the source collection's View permission and exact membership in
  `products.image_url`/`products.images`, `categories.image_url` or
  `media_assets.url`. Only an object in the configured StoreMink GCS bucket is
  converted to a `gs://` image part for Vertex; arbitrary web URLs never reach
  the provider.
- **REFERENCES HAVE SOURCE-SPECIFIC MEANING.** The executable Markdown prompt
  tells the image model to preserve a referenced product's visible identity,
  form, colours and packaging; keep a category image recognisably about that
  category; and use a Media Library image's subject, composition, palette or
...
- **THIS CREATES NEW STOREFRONT ARTWORK; IT DOES NOT CHANGE ITS SOURCES.** A
  ...
  and may be used in a separately approved page layout. Mink still has no tool
  that writes `products.images`, and it never calls generated artwork the
  original catalogue photograph. Existing images are neither overwritten nor
  deleted.
- `20260920_0118_mink_reference_image_help` edits the existing merchant-facing
  picture paragraph in place so it explains how to name a product, category or
  Media image as a visual reference. It adds no Help heading.
...
**★★ THE LAST PERMITTED MODEL TURN MAY FINISH ONE EXPLICITLY MARKED REVIEW
PROPOSAL (fixed 2026-09-20).** The post-fix production trace for “Create the
banner on the homepage carousel for this buy 1 get 1 offer” still failed: 12
low-thinking model turns, 15 successful tool calls and 253,339 total tokens.
The eleventh tool round successfully saved the generated image; the model's
twelfth turn returned another function call, but the orchestrator checked
`steps >= maxSteps` at the TOP of the loop and rejected it before recording even
its name. Telemetry therefore cannot claim which call was discarded; what it
does prove is that a final `propose_storefront_layout` selected in that position
could never finish. Raising every run's global step cap would re-bill the
~11k-token prefix for every unrelated long request. Instead
`MinkTool.stepLimitCompletionText` is an opt-in finalizer: exactly one marked
tool may execute on the last permitted turn, only a successful produced review
artifact completes the run, and the orchestrator uses deterministic success
copy instead of buying a thirteenth prose-only model turn. Only
`propose_storefront_layout` opts in. Reads, image generation, workflows, live
actions, multiple calls, tool errors and missing artifacts still fail at the
same cap; the 16-tool ceiling and 12-turn setting are unchanged. The regression
test reproduces the observed first 11 rounds and 15 calls, supplies the expected
layout proposal as call 16, and asserts only 11 tool-response model calls, so
removing the escape hatch or adding a final provider turn fails it.

The same trace made four successful `search_help_centre` calls while planning
the already-declared layout action, plus `list_storefront_pages` after the exact
home page context was already available. The runtime prompt and Help declaration
now reserve Help retrieval for a merchant's own how-to, navigation, setup,
...
│                              # postcondition contracts and the two mistakes that have
│                              # each jammed the queue, the troubleshooting table, and the
│                              # stated gaps — above all that there is NO reproducible
│                              # fresh-database build, because drizzle/manual is a
│                              # 43-table Phase-5 snapshot (no billing_*, no
│                              # store_locations) and the manifest baseline is far later,
│                              # so CI cannot replay the ledger from scratch.
...
│                              # the deploy: (1) the older image lands over the newer one —
│                              # a silent rollback, both builds green — and (2) the older
│                              # build's migrate reads a ledger holding a migration its own
│                              # checkout has never seen, so assertHealthyPlan throws
│                              # "unknown migrations" at a developer who did nothing wrong.
│                              # It writes /workspace/superseded and migrate+deploy no-op
│                              # on it, finishing GREEN having changed nothing —
│                              # deliberately, since a routinely red pipeline is one nobody
│                              # reads. ⚠ It runs AFTER the image build, not before: the
│                              # race is the tip moving WHILE the slow build runs, so an
│                              # earlier check answers a question that is stale by the time
│                              # it matters. ⚠ A failed `git fetch` PROCEEDS (we cannot tell
│                              # "superseded" from "GitHub blinked", and a merge that looks
│                              # deployed and is not is the worse half), but a failed marker
│                              # WRITE fails the build (proceeding would deploy the stale
│                              # image this step exists to stop). ⚠ `deploy` became
│                              # `entrypoint: bash` to be able to no-op; its ten gcloud
│                              # flags are unchanged.
...
│   │                          # system-prompt.ts and image-prompt.ts validate/render the
│   │                          # marked runtime templates in docs/mink-ai-system-prompt.md
│   │                          # and docs/mink-ai-image-prompt.md; trusted actor,
...
│                          # image-prompt.ts (validated Markdown prompt) and
│                          # media-reference-images.ts (tenant/permission/source
│                          # recheck plus exact GCS image-part resolution),
...
│   ├── migration-lint.mjs     # ★ THE BACKWARD-COMPATIBILITY GATE (docs/migrations.md).
│   │                          # Cloud Run serves old and new containers against ONE
│   │                          # database for the length of a rollout, so a migration
│   │                          # that drops/renames/retypes a column, adds NOT NULL with
│   │                          # no default, SET NOT NULLs an existing column or
│   │                          # TRUNCATEs breaks the revision it is replacing — with no
│   │                          # error, because both halves did as told. Blocks the
│   │                          # mechanical half of expand/contract in the PR.
│   │                          # ★ LINTS THE WHOLE MANIFEST, NOT A GIT DIFF: a
│   │                          # diff-scoped linter needs branch context, differs between
│   │                          # local and CI, and silently checks nothing when the base
│   │                          # ref is missing. Affordable only because all 98 existing
│   │                          # migrations pass — the one exception predating the linter
│   │                          # is named in GRANDFATHERED with its reason, because its
│   │                          # SQL cannot be edited to carry a marker (that would
│   │                          # change an applied checksum). New work uses an in-SQL
│   │                          # `-- migration-lint: allow <rule> — <reason>` marker; the
│   │                          # reason is REQUIRED and lands in the diff. ⚠ Regex
│   │                          # statement shapes, not a parser — expand/contract stays a
│   │                          # human discipline. `npm run db:lint`; tested both ways,
│   │                          # including the false-positive set (a linter that flags
│   │                          # ordinary additive DDL is one somebody disables).
...
│   │                          # ★★ THE ADVISORY LOCK NOW HAS A DEADLINE, and each
│   │                          # migration runs under `SET LOCAL lock_timeout`
│   │                          # (MIGRATION_LOCK_WAIT_SECONDS 300, default clamped;
│   │                          # MIGRATION_LOCK_TIMEOUT_MS 5000). `pg_advisory_lock`
│   │                          # blocks with NO deadline — right at a terminal, wrong in
│   │                          # a build, where the only thing that ends the wait is
│   │                          # Cloud Build's own timeout, reported as "build timed out"
│   │                          # and naming neither the lock nor the other build. And an
│   │                          # ALTER TABLE that cannot take its lock does not fail, it
│   │                          # QUEUES — with every later query for that table behind
│   │                          # it, which is a full outage caused by a deploy.
│   │                          # lock_timeout turns that into a failed build with nothing
│   │                          # half-applied. ⚠ The timeout is INTERPOLATED (SET takes
│   │                          # no bind parameters), so `lockTimeoutMs()` returning a
│   │                          # bounded integer IS the injection boundary — tested with
│   │                          # a `3000; drop table stores` value.
│   │                          # ⚠ The lock is NOT what makes double-apply impossible:
│   │                          # schema_migrations.id is a PK and the ledger insert
│   │                          # shares the migration's transaction, so a race loser hits
│   │                          # a duplicate key and its DDL rolls back with it. Two
│   │                          # independent layers, which is why bounding the wait is
│   │                          # safe.
│   ├── db-migrations-core.mjs # pure manifest/checksum/planning primitives + the lock
│   │                          # helpers (tested — they live here, not in db-migrate.mjs,
│   │                          # because that file calls main() on import and cannot be
│   │                          # imported by a test). One MIGRATION_LOCK_NAME constant,
│   │                          # imported by the runner: two string literals is how a
│   │                          # lock stops protecting anything.
...
required placeholders before injecting only trusted actor/tool context.
`docs/mink-ai-image-prompt.md` is likewise the executable marked prompt for the
storefront image provider; `lib/mink/image-prompt.ts` validates its scene and
server-authored reference-guidance placeholders and renders them without
reinterpreting merchant text. Next output
tracing and narrow Docker-context exceptions ship both Markdown files with the
standalone server.
...
Its read tools cover store profile, catalogue summary, product and category
image search,
